...
const WhoCards = () => {
  return team.map((member, i) => (
    <a
      key={i}
      className="flex items-center flex-col gap-[30px] w-[360px] min-h-[432px] group"
      href={member.webUrl}
      target="_blank"
    >
      ...
      />
      <h4 className="text-night text-[22px] text-black-olive group-hover:text-[24px] transition-all font-aldelphi">
        {member.role}
      </h4>
    </a>
  ));
